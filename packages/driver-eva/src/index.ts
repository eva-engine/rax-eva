...
const ListeningProps: any[] = [
  ['alpha', 'zIndex', 'visible', 'sortableChildren'], Render // Render Component & It's filter
];
...
function findComponent(key): typeof Component {
  ...
  for (let i = 0; i < ListeningProps.length; i += 2) {
    ...
    if (filter instanceof Array && filter.indexOf(key) > -1
...
function processListeningProps(gameObject: GameObject, restProps) {
  const map: Record<string, {ComponentClass: typeof Component, InitParams: Record<string, any>}> = {};
...
function setEvaElement(el) {
  if (!(el instanceof GameObject || el instanceof TextNode || el instanceof EmptyNode)) {
...
export default class Driver {
  ...
  createText(text, raxComponent) {
    ...
    const node = new TextNode(`text${textComponent._textId}`, textComponent, this._textMap[textId].length - 1);
